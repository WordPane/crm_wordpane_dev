import { eq } from "drizzle-orm";
import { NextResponse } from "next/server";

import {
  assertCompanyAccess,
  ForbiddenError,
  getSessionUser,
  isTeam,
} from "@/lib/access/permissions";
import { AsaasError, getPixQrCode } from "@/lib/asaas/client";
import { db } from "@/lib/db";
import { charges } from "@/lib/db/schema";

/**
 * GET /api/financeiro/[id]/pix-qrcode — QR code PIX da cobrança.
 * Equipe com acesso à empresa ou cliente da mesma empresa.
 */
export async function GET(
  _request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  const user = await getSessionUser();
  if (!user) {
    return NextResponse.json({ error: "Não autenticado." }, { status: 401 });
  }

  const { id } = await params;

  const [charge] = await db
    .select()
    .from(charges)
    .where(eq(charges.id, id))
    .limit(1);
  if (!charge || !charge.asaasPaymentId) {
    return NextResponse.json(
      { error: "Cobrança não encontrada." },
      { status: 404 },
    );
  }

  if (isTeam(user.role)) {
    try {
      await assertCompanyAccess(user, charge.companyId);
    } catch (error) {
      if (error instanceof ForbiddenError) {
        return NextResponse.json({ error: error.message }, { status: 403 });
      }
      throw error;
    }
  } else if (user.role !== "client" || user.companyId !== charge.companyId) {
    return NextResponse.json(
      { error: "Você não tem permissão para esta ação." },
      { status: 403 },
    );
  }

  if (charge.billingType !== "pix") {
    return NextResponse.json(
      { error: "Esta cobrança não é PIX." },
      { status: 400 },
    );
  }

  try {
    const qr = await getPixQrCode(charge.asaasPaymentId);
    return NextResponse.json({
      image: qr.encodedImage,
      payload: qr.payload,
      expirationDate: qr.expirationDate,
    });
  } catch (error) {
    if (error instanceof AsaasError) {
      return NextResponse.json({ error: error.message }, { status: 502 });
    }
    throw error;
  }
}
